...
const DOC_TEXT = {
  sw: {
    ...
    date: "Tarehe",
    deliveryNoteTitle: "DELIVERY NOTE / HATI YA UWASILISHAJI",
    driver: "Jina la Dereva",
    vehicleNo: "Namba ya Gari",
    itemsDelivered: "Bidhaa/Huduma Zilizowasilishwa",
    deliveredBy: "Aliyepeleka (Dereva)",
    receivedBy: "Aliyepokea (Mteja)",
    deliveryNote: "Hii ni uthibitisho wa uwasilishaji wa bidhaa/huduma, sio invoice ya malipo.",
  },
  en: {
    client: "Client",
    phone: "Phone",
    ...
    itemsDelivered: "Items/Services Delivered",
    deliveredBy: "Delivered By (Driver)",
    receivedBy: "Received By (Client)",
    deliveryNote: "This confirms delivery of goods/services. This is not a payment invoice.",
  },
}
...
function paymentInfoHtml(lang, details) {
  ...
  if (info.service) {
    qrText += " | " + (lang === "en" ? "Service" : "Huduma") + ": " + info.service
  }
  if (info.amount) {
...
export default function Invoices() {
  ...
  function downloadDeliveryNote() {
    ...
    if (validItems.length === 0) {
      setMessage({ type: "error", text: "Ongeza angalau kitu kimoja kabla ya kupakua delivery note" })
      return
    }

    const now = new Date()
    const stamp = now.getFullYear().toString() + (now.getMonth() + 1).toString().padStart(2, "0") + now.getDate().toString().padStart(2, "0")
    const dnNumber = "DN-" + stamp + "-" + Math.floor(Math.random() * 9000 + 1000)
    const logoUrl = window.location.origin + "/logo.png"
    ...
    const html =
      "<html><head><title>" + dnNumber + "</title>" +
      "<style>" +
      "body{font-family:Arial,Helvetica,sans-serif;padding:36px;color:#1a1a1a;}" +
      ".header{display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #1D9E75;padding-bottom:16px;margin-bottom:24px;}" +
      ".header img{height:60px;}" +
      ...
      "th{text-align:left;background:#f3f6f4;}" +
      ".info p{margin:4px 0;font-size:13px;}" +
      ".note{margin-top:16px;font-size:12px;color:#085041;background:#e1f5ee;padding:10px;border-radius:6px;}" +
      ...
      "</style></head><body>" +
      companyHeaderHtml(logoUrl, docLang) +
      "<h2>" + t.deliveryNoteTitle + " &mdash; " + dnNumber + "</h2>" +
      "<div class='info'>" +
      "<p><strong>" + t.client + ":</strong> " + (client ? client.name : "") + "</p>" +
      "<p><strong>" + t.service + ":</strong> " + (serviceLine ? serviceLine.name : "") + "</p>" +
      "<p><strong>" + t.date + ":</strong> " + now.toLocaleDateString("en-GB") + "</p>" +
      "<p><strong>" + t.driver + ":</strong> " + (deliveryInfo.driver_name || "-") + "</p>" +
      "<p><strong>" + t.vehicleNo + ":</strong> " + (deliveryInfo.vehicle_number || "-") + "</p>" +
      "</div>" +
      "<h2 style='font-size:14px;'>" + t.itemsDelivered + "</h2>" +
      "<table><thead><tr><th>" + t.description + "</th><th>" + t.qty + "</th></tr></thead>" +
      "<tbody>" + rowsHtml + "</tbody></table>" +
      "<p class='note'>" + t.deliveryNote + "</p>" +
      "<div class='sign-section'>" +
      "<div class='sign-box'>" + t.deliveredBy + "</div>" +
      "<div class='sign-box'>" + t.receivedBy + "</div>" +
      "</div>" +
      "<p class='footer'>AJ PLUS COMPANY LIMITED</p>" +
      "<script>window.onload = function(){ window.print(); }</script>" +
      "</body></html>"
      ...
      <div className="panel">
        ...
        {showForm ? (
          <form className="income-form" onSubmit={handleSubmit}>
            ...
            <div className="form-row">
              <label>
                Jina la Dereva (kwa Delivery Note)
                <input
                  type="text"
                  placeholder="mfano: Juma Said"
                  value={deliveryInfo.driver_name}
                  onChange={function (e) {
                    setDeliveryInfo(function (d) { return Object.assign({}, d, { driver_name: e.target.value }) })
                  }}
                />
              </label>
              <label>
                ...
                <input
                  ...
                  value={deliveryInfo.vehicle_number}
                  onChange={function (e) {
                    setDeliveryInfo(function (d) { return Object.assign({}, d, { vehicle_number: e.target.value }) })
                  }}
                />
              </label>
            </div>
            ...
            <div className="header-buttons">
              ...
              <button type="button" className="btn-cancel" onClick={downloadDeliveryNote}>
                Pakua Delivery Note (PDF)
              </button>
              <button className="btn-approve submit-income" disabled={busy}>
